   ...
   const tooltipList = tooltipTriggerList.map(function(tooltipTriggerEl) {
   	...
   	$(document).ready(function() {
		$('#dataTable').DataTable({
			responsive: true,
			columnDefs: [
				{ responsivePriority: 1, targets: 1 },  // Name (most important)
			...
			order: [[0, 'asc']], // Sort by the counter column
			paging: true,
			scrollY: '400px',
			scrollX: true,
			scrollCollapse: true,
			fixedHeader: true, // Keep headers visible
			language: {
				search: "<i class='fas fa-search'></i> Search:",
				lengthMenu: "Show _MENU_ entries",
				info: "Showing _START_ to _END_ of _TOTAL_ features",
				emptyTable: "No features found" // Added for empty state
			},
