...
function check_and_create_overdue_fines($connect) {
    ...
    $query = "
        ...
        WHERE 
            ...
            AND DATEDIFF(CURRENT_DATE, ib.expected_return_date) > 0
    ";
...
// Handle "Add Fine for Lost Book" action if submitted
if (isset($_POST['add_lost_fine'])) {
    $issue_book_id = $_POST['issue_book_id'];
    $fine_amount = $_POST['fine_amount'];
    $user_id = $_POST['user_id'];
    
    // Check if this issue already has a fine
    $check_query = "SELECT fines_id FROM lms_fines WHERE issue_book_id = :issue_book_id";
    $check_statement = $connect->prepare($check_query);
    $check_statement->bindParam(':issue_book_id', $issue_book_id);
    $check_statement->execute();
    
    if ($check_statement->rowCount() > 0) {
        $message = 'A fine already exists for this lost book.';
    } else {
        // Insert new fine for lost book
        $insert_query = "
            INSERT INTO lms_fines 
            (user_id, issue_book_id, fines_amount, fines_status, fines_created_on, fines_updated_on) 
            VALUES 
        ...
        $insert_statement->bindParam(':issue_book_id', $issue_book_id);
        $insert_statement->bindParam(':fines_amount', $fine_amount);
        ...
        if ($insert_statement->execute()) {
            ...
            $message = 'Failed to add fine for lost book.';
        }
    }
}
...
<!-- Lost Book Fine Modal -->
<div class="modal fade" id="addLostFineModal" tabindex="-1" aria-labelledby="addLostFineModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-danger text-white">
                <h5 class="modal-title" id="addLostFineModalLabel">Add Fine for Lost Book</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="mb-3">
                    <input type="text" class="form-control" id="lostBookSearch" placeholder="Search by User Name or Issue ID">
                </div>
                
                <?php if (empty($lost_books)): ?>
                    <div class="alert alert-info">No lost books without fines found.</div>
                <?php else: ?>
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped" id="lostBooksTable">
                            <thead>
                                <tr>
                                    <th>Issue ID</th>
                                    <th>User</th>
                            ...
                            <tbody>
                                <?php foreach ($lost_books as $book): ?>
                                    <tr>
                                        <td><?= htmlspecialchars($book['issue_book_id']) ?></td>
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-sm btn-warning select-lost-book"
                                                data-issue-id="<?= htmlspecialchars($book['issue_book_id']) ?>"
                                                data-user-id="<?= htmlspecialchars($book['user_id']) ?>"
                                                data-user-name="<?= htmlspecialchars($book['user_name']) ?>"
                                                data-book-name="<?= htmlspecialchars($book['book_name']) ?>">
                                                Select
                                            </button>
                                        </td>
                                    </tr>
                ...
                <form id="addLostFineForm" action="" method="post" class="mt-3" style="display: none;">
                    <input type="hidden" name="issue_book_id" id="selected_issue_id">
                    <input type="hidden" name="user_id" id="selected_user_id">
                    
                    <div class="mb-3">
                        <label class="form-label">Selected Book:</label>
                        <input type="text" class="form-control" id="selected_book_name" readonly>
                    </div>
                    
                    <div class="mb-3">
                        <label class="form-label">User:</label>
                        <input type="text" class="form-control" id="selected_user_name" readonly>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" class="form-control" id="fine_amount" name="fine_amount" min="0" step="0.01" required>
                        </div>
                    </div>
                    
                    <div class="d-grid">
                        <button type="submit" name="add_lost_fine" class="btn btn-primary">Add Fine</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="card-header bg-white py-3">
        ...
        <h5 class="mb-0"><i class="fas fa-table me-2"></i>Fines List</h5>                      
                    <?php if (!isset($_GET['action'])): ?>            
                        <a href="fines.php?action=add" class="btn btn-sm btn-success">
